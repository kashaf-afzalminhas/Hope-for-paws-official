...
export default function SellerOrderDashboard() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const token = localStorage.getItem('token') || localStorage.getItem('hope_for_paws_token');
        ...
        <header className="flex items-start justify-between mb-10 gap-4">
          <div>
            ...
            <p className="text-sm text-stone-500 mt-1.5 font-medium">
              {orders.length} total orders · <span className="text-emerald-600 font-bold">Rs {totalRevenue.toLocaleString()}</span> revenue
            </p>
          </div>
