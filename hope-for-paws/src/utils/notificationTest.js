...
export const testNotificationSystem = async () => {
  console.log('Ã°Å¸â€Â Testing notification system...\n');
  ...
  try {
    ...
    console.log('Ã¢Å“â€¦ Health endpoint working:', healthResponse.data);
    ...
    console.log('Ã¢Å“â€¦ Socket health endpoint working:', socketHealthResponse.data);
    ...
    try {
      ...
      console.log('Ã¢Å“â€¦ Notification test endpoint working:', testResponse.data);
    } catch (error) {
      console.log('Ã¢ÂÅ’ Notification test endpoint error:', error.response?.status, error.response?.data?.message);
    }
    ...
    if (token && user) {
      console.log('Ã¢Å“â€¦ User is logged in:', user.username);
      console.log('Ã¢Å“â€¦ Token exists:', token.substring(0, 20) + '...');
    } else {
      console.log('Ã¢ÂÅ’ User not logged in');
    ...
    try {
      ...
      console.log('Ã¢Å“â€¦ Notifications endpoint working:', notificationsResponse.data);
    } catch (error) {
      console.log('Ã¢ÂÅ’ Notifications endpoint error:', error.response?.status, error.response?.data?.message);
    ...
    try {
      ...
      console.log('Ã¢Å“â€¦ Unread count endpoint working:', unreadResponse.data);
    } catch (error) {
      console.log('Ã¢ÂÅ’ Unread count endpoint error:', error.response?.status, error.response?.data?.message);
      console.log('Full error:', error);
    }
    ...
    try {
      ...
      console.log('Ã¢Å“â€¦ Socket.IO connection successful:', result);
      socket.disconnect();
    } catch (error) {
      console.log('Ã¢ÂÅ’ Socket.IO connection failed:', error.message);
      console.log('Note: This is expected on Vercel as it doesn\'t support WebSocket connections');
    }

    console.log('\nÃ¢Å“â€¦ Notification system test completed!');
    console.log('\nSummary:');
    ...
    console.error('Ã¢ÂÅ’ Test failed:', error.message);
    if (error.response) {
