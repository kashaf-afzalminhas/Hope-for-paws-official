...
const UserCard = ({
  ...
  return (
    <div
      className={cn(
        'flex items-center p-4 gap-4 cursor-pointer rounded-xl transition-all duration-200',
      ...
      >
        {user.profileImage ? (
          <img 
            src={`${AUTH_BASE_URL.replace('/auth', '')}${user.profileImage}`}
            alt={user.username || 'User'} 
            className="w-14 h-14 rounded-xl object-cover border border-[#a07855]/10"
            onError={(e) => {
              e.target.style.display = 'none';
              e.target.nextSibling.style.display = 'flex';
            }}
        ...
        <div 
          className={`w-14 h-14 rounded-xl bg-gradient-to-br from-[#a07855] to-[#6b493d] flex items-center justify-center text-[#ffd8b8] text-xl font-bold ${user.profileImage ? 'hidden' : ''}`}
        >
          {(user.username || 'U').charAt(0).toUpperCase()}
        </div>
        {/* Online status indicator */}
      ...
      <div className="flex-1 min-w-0 overflow-hidden">
        ...
        <div className="flex justify-between items-center mt-1.5 gap-2">
          ...
          {unreadCount > 0 && (
            <span className={cn(
              "bg-[#a07855] text-[#ffd8b8] text-xs font-bold rounded-full",
              "min-w-[22px] h-[22px] flex items-center justify-center shrink-0",
